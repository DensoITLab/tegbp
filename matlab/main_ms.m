...
%% node
nodes           = define_nodes_ms(h, w, n_scale);
...
for e = 1 : n_event
    ...
    if ti > t_frame_list(i_frame) || e == n_event
        ...
        flow_est    = permute(nodes{1}.marg(1:2, pad_size+1:end-pad_size, pad_size+1:end-pad_size), [2, 3, 1]);
        flow_norm   = permute(flow_norm_(:, pad_size+1:end-pad_size, pad_size+1:end-pad_size), [2, 3, 1]);
        t_act       = nodes{1}.t_act(pad_size+1:end-pad_size, pad_size+1:end-pad_size);
    ...
    if cnt >= n_batch 
        ...
        for i = 1 : n_batch
            ...
            %% setting data factor
            nodes       = set_ms_nodes(nodes, y, x, t, obs_msg, dt_act);

            %% message passing
            nodes       = message_passing_event_ms(nodes, y, x, t, dt_act, Lam_p, huber, n_scale);
        end
